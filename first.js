...
OPTIONS["FullBoardSize"]=OPTIONS["FullBlockSize"]*OPTIONS["BoardSize"];

if (parseInt(($( document ).width()-OPTIONS["FullBoardSize"])/2)>250) {
	OPTIONS["Offset"]=parseInt(($( document ).width()-OPTIONS["FullBoardSize"])/2);
}else{
	OPTIONS["Offset"]=0;
}
...
Board.prototype.draw = function(){
	...
	for (var x = OPTIONS["BoardSize"]-1 ; x >= 0; x--) {
		for (var y =OPTIONS["BoardSize"]-1 ; y >= 0; y--) {
			var style='style="left:'+(OPTIONS["Offset"]+OPTIONS["FullBlockSize"]*x)+'px;top:'+(OPTIONS["FullBlockSize"]*y)+'px;"';
...
function Figur (board,name){
	...
	this.board=board;
	this.block_size=parseInt(OPTIONS["FullBlockSize"]/1.3);
	this.number;
	this.kind=-1;
	this.blocks=[];
};
...
Figur.prototype.blocks_combimation	= 	[	
											[ [2,1] , [2,2] ] ,// | 2
											[ [2,1] , [2,2] , [2,3] ] ,// | 3
											[ [2,0] , [2,1] , [2,2] , [2,3] ] ,// | 4
...
Figur.prototype.draw = function(number){
	...
	var offset_left=OPTIONS["Offset"]+OPTIONS["FullBoardSize"]*((1/3)*this.number);
	var offset_top=this.block_size;
	
	$(".figurs").append('<div class='+this.class_name+'></div>');

	for (var i in this.blocks) {
		var style='style="left:'+parseInt((offset_left+this.block_size*this.blocks[i][0]))+'px;top:'+parseInt((offset_top+OPTIONS["FullBoardSize"]+this.block_size*this.blocks[i][1]))+'px;"';
...
Figur.prototype.upset=function(){
	...
	for (var i in this.blocks) {
		$("#"+this.blocks[i][0]+'_'+this.blocks[i][1]+'_'+this.number).css({	left:		parseInt(offset_left+this.board.block_size*this.blocks[i][0]),
																				top:		parseInt(offset_top+OPTIONS["FullBoardSize"]+this.board.block_size*this.blocks[i][1])});
...
Figur.prototype.downset=function(){
	...
	for (var i in this.blocks) {
		$("#"+this.blocks[i][0]+'_'+this.blocks[i][1]+'_'+this.number).css({	left:		parseInt(offset_left+this.block_size*this.blocks[i][0]),
																				top:		parseInt(offset_top+OPTIONS["FullBoardSize"]+this.block_size*this.blocks[i][1])});
	};
	$("."+this.class_name+" > .block_move").css({width:this.block_size,height:this.block_size})
}
...
$(document).ready(function() {
	...
	$("#points").css({	left:		OPTIONS["FullBoardSize"]+OPTIONS["Offset"]+OPTIONS["FullBlockSize"],
						top:		OPTIONS["FullBlockSize"]
					});
	$("#restart").css({	left:		OPTIONS["FullBoardSize"]+OPTIONS["Offset"]+OPTIONS["FullBlockSize"],
						top:		OPTIONS["FullBlockSize"]*3
					});

	$(function() {
		$('.'+OPTIONS["BlockClassName"]).droppable({
